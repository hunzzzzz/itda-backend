package com.moira.itda.global.entity

import com.moira.itda.domain.trade.add.dto.request.ExchangeItemAddRequest
import com.moira.itda.domain.trade.add.dto.request.SalesItemAddRequest
import java.time.ZonedDateTime
import java.util.*

data class TradeItem(
    val id: String,
    val tradeId: String,
    val gachaId: String,
    val type: TradeItemType,
    val status: TradeItemStatus,
    val createdAt: ZonedDateTime,
    val updatedAt: ZonedDateTime,
    // 구매 관련
    val salesItemId: Long?,
    val salesPrice: Int?,
    // 교환 관련
    val exchangeGiveItemId: Long?,
    val exchangeWantItemId: Long?
) {
    companion object {
        fun from(tradeId: String, gachaId: String, request: SalesItemAddRequest): TradeItem {
            return TradeItem(
                id = UUID.randomUUID().toString(),
                tradeId = tradeId,
                gachaId = gachaId,
                type = TradeItemType.SALES,
                status = TradeItemStatus.PENDING,
                createdAt = ZonedDateTime.now(),
                updatedAt = ZonedDateTime.now(),
                salesItemId = request.gachaItemId,
                salesPrice = request.price,
                exchangeGiveItemId = null,
                exchangeWantItemId = null,
            )
        }

        fun from(
            tradeId: String,
            gachaId: String,
            request: ExchangeItemAddRequest
        ): TradeItem {
            return TradeItem(
                id = UUID.randomUUID().toString(),
                tradeId = tradeId,
                gachaId = gachaId,
                type = TradeItemType.EXCHANGE,
                status = TradeItemStatus.PENDING,
                createdAt = ZonedDateTime.now(),
                updatedAt = ZonedDateTime.now(),
                salesItemId = null,
                salesPrice = null,
                exchangeGiveItemId = request.giveItemId,
                exchangeWantItemId = request.wantItemId,
            )
        }
    }
}